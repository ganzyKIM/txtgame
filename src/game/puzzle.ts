...
const DIFFICULTY_GUIDE: Record<Difficulty, string> = {
  easy: [
    '【쉬움 — 정답 선택 기준】',
    '  ① 초등학생 혹은 평범한 성인 10명 중 8명 이상이 이름만 들으면 바로 아는 대상이어야 한다.',
    '  ② 해당 카테고리에서 가장 먼저 떠오르는 대표급(예: 세종대왕, 에펠탑, 피카추, 방탄소년단, 스파게티)으로 골라라.',
    '  ③ 너무 흔하다 싶어도 괜찮다 — "쉬움"은 누구나 맞힐 수 있어야 의미가 있다.',
    '  → maxHints는 6~8로 아주 넉넉하게 잡아라.',
  ].join('\n'),
  normal: [
    '【보통 — 정답 선택 기준】',
    '  ① 해당 분야에 관심 있는 일반인 10명 중 5명 이상이 힌트 몇 개면 맞힐 만한 것.',
    '  ② 너무 희귀하거나 마니아만 아는 것은 피하고, TV/미디어에서 한 번쯤 다뤄진 수준.',
    '  → maxHints는 5~7로 잡아라.',
  ].join('\n'),
...
const OTAKU_DIFFICULTY_GUIDE: Record<Difficulty, string> = {
  easy: [
    '【쉬움(오타쿠 카테고리) — 정답 선택 기준】',
    '  플레이어는 이미 오타쿠/팬이다. 팬이라면 힌트 없이도 대뜸 떠오를 만한 최상위 대표작·대표 캐릭터만 골라라.',
    '  ① 원피스·나루토·드래곤볼·포켓몬·귀멸 급의 국민적 IP.',
    '  ② 이 카테고리를 1년 이상 즐긴 팬 10명 중 9명이 바로 아는 것.',
    '  → maxHints는 6~8로 아주 넉넉하게 잡아라.',
  ].join('\n'),
  normal: [
    '【보통(오타쿠 카테고리) — 정답 선택 기준】',
    '  팬 사이에서 유명하지만 일반인은 모를 수도 있는 "세컨드티어"급으로 골라라.',
    '  ① 활발한 팬덤이 있고, 관련 위키·팬카페가 존재하며, 해당 분야 팬 10명 중 6명 이상이 아는 것.',
    '  ② "유명하긴 한데 힌트 없이는 바로 안 떠오르는" 수준.',
    '  → maxHints는 5~7로 잡아라.',
  ].join('\n'),
...
export const PROMPT_VERSION = 'v2-cache';
...
export function buildSetupPrompt(categoryLabel: string, theme: string, difficulty: Difficulty, recentAnswers: string[] = [], categoryDetail = '', categoryKey = '', diffCalib = '', axes?: GenAxes, failurePatterns: string[] = []): string {
  ...
  return [
    '너는 추리 퀴즈 게임의 출제자다. 맨 아래【이번 출제 조건】으로 단 하나의 정답을 비밀리에 정하고, 그 정답을 맞히기 위한 힌트들을 만든다.',
    '',
    '━━━━━━━━━━━━━━━━━━━━━━━━',
    '【0순위 규칙 — 위키백과 실존 검증】',
    ...
    '2. 대중성: "해당 카테고리를 좋아하는 일반인 대다수가 이름을 들으면 바로 아는" 수준이어야 한다. 마니아·전문가만 아는 딥컷, 소규모 인디·지역 팀, 검색해도 정보가 거의 없는 대상 금지. (단, 난이도가 어려움이면 난이도 지침을 우선한다.)',
    ...
    '· 카테고리 노출 금지: 플레이어는 자기가 고른 카테고리를 이미 안다. "이 카테고리의 ~입니다", "한국/일본의 유명한 ~입니다"처럼 카테고리 선택만으로 자명한 내용 금지. 정답을 좁히는 구체적 특징만.',
