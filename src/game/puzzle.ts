...
const DIFFICULTY_GUIDE: Record<Difficulty, string> = {
  ...
  hard: '【어려움 전용】 누구나 아는 유명 정답(1순위·대표작·국민급)은 이 난이도에서 절대 출제 금지다. 해당 분야를 꽤 아는 사람도 힌트 없이는 바로 못 맞힐 수준의 정답만 허용. 힌트는 함축적으로, maxHints는 3~4로 빡빡하게.',
};
...
const OTAKU_DIFFICULTY_GUIDE: Record<Difficulty, string> = {
  ...
  hard:   '【어려움 전용】 원피스·나루토·드래곤볼 같은 메이저 작품, 누구나 아는 대표 캐릭터는 절대 출제 금지. 코어 팬도 힌트 없이 바로 못 맞힐 만큼 세분화된 정답(마이너 캐릭터, 비주류 작품, 특정 설정·용어 등)을 골라라. 단 위키백과(ko 또는 en) 독립 항목은 반드시 있어야 한다. 힌트는 함축적으로, maxHints는 3~4.',
};
...
export function buildSetupPrompt(categoryLabel: string, theme: string, difficulty: Difficulty, recentAnswers: string[] = [], categoryDetail = '', categoryKey = ''): string {
  ...
  return [
    ...
    '',
    '════════════════════════════════════════',
    '★★★ 최우선 절대 규칙 — 위키백과 실존 검증 ★★★',
    '정답을 고르기 전, 반드시 스스로 아래를 확인하라:',
    '  → 한국어 위키백과(ko.wikipedia.org) 또는 영어 위키백과(en.wikipedia.org)에',
    '     이 정답만을 다루는 독립 문서가 실제로 존재하는가?',
    '  → YES면 출제 가능. NO면 즉시 다른 정답으로 교체하라.',
    '이 규칙은 초성·다양성·난이도 등 모든 다른 지침보다 우선한다.',
    '위키백과에 없는 정답은 어떤 이유로도 허용되지 않는다.',
    '════════════════════════════════════════',
    '',
    `[카테고리] ${categoryLabel}`,
    ...
    '   • [answer 표기 기준] answer 필드는 반드시 한국어 위키백과 문서 제목 기준의 공식 풀네임으로 써라. 약칭·별명·성만 쓰는 것은 금지. (예: "미쿠" X → "하츠네 미쿠" O, "손흥민" O, "아이유" O)',
    '2. [추측 가능성 — 가장 중요한 규칙] 정답은 반드시 "해당 카테고리를 좋아하는 일반인 대다수가 이름을 들으면 바로 알 수 있는" 수준의 대중적으로 유명한 대상이어야 한다.',
    '   • 기준: 한국 기준 포털 검색 자동완성에 뜨거나, TV·유튜브·SNS에서 자주 언급되거나, 비팬도 이름은 들어봤을 정도.',
    '   • 절대 금지: 해당 분야 마니아·전문가만 아는 딥컷, 소규모 인디·지역 공연 팀, 학술·전통예술 분야 비주류 단체, 검색해도 정보가 거의 안 나오는 대상.',
    '   • 자가검증: 정답을 정하기 전에 "이 정답을 모르는 사람이 힌트를 다 봐도 납득할 수 있나?"를 스스로 물어봐라. "이런 게 있었어?" 반응이 예상되면 다른 정답을 골라라.',
    '3. hints는 6~8개. "넓고 모호한 단서 → 점점 구체적·결정적인 단서" 순서로 정렬한다. 각 힌트는 카테고리를 되풀이 설명하는 데 그치지 말고, 정답을 좁혀 가는 "구체적 단서"(대표작·연도·수상·외형·대사·소속 등 집어낼 수 있는 사실)를 담아야 한다.',
    '4. [힌트 작성 원칙]',
    ...
    `   ⑤ [카테고리 금지] 플레이어는 카테고리("${categoryLabel}")를 이미 알고 있다. "이 카테고리의 ~입니다", "${categoryLabel}입니다", "한국/일본의 유명한 ~입니다" 처럼 카테고리 선택만으로 자명하게 알 수 있는 내용은 힌트에 절대 쓰지 마라. 카테고리 안에서 정답을 좁히는 구체적 특징만 써라.`,
    ...
    '',
    '[출력 전 필수 자가검증 — 전부 통과해야만 JSON 출력 가능]',
    '  ✔ 체크 1. 【위키백과】 ko.wikipedia.org 또는 en.wikipedia.org에 이 정답만 다루는 독립 문서가 존재하는가? → 없으면 정답을 즉시 바꿔라. 이 체크를 통과하지 못하면 나머지 체크는 볼 필요도 없다.',
    '  ✔ 체크 2. 【난이도 적합성】 선택한 난이도에 맞는 정답인가? 어려움인데 누구나 아는 정답이면 바꿔라.',
    '  ✔ 체크 3. 【금지 목록】 이 정답이 이미 출제된 목록에 없는가? → 있으면 정답을 바꿔라.',
