...
export function buildSetupPrompt(categoryLabel: string, theme: string, difficulty: Difficulty, recentAnswers: string[] = [], categoryDetail = ''): string {
  ...
  return [
    ...
    factStrict
      ...
      : '  • 매번 똑같은 1순위 정답만 기계적으로 고르지 말고, 충분히 알려진 실존 대상들 중에서 폭넓게 골라라.',
    '',
    '규칙:',
    '1. 정답(answer)은 현실에 실제로 존재하는, 고유명사로 특정되는 "단 하나의 개별 대상"이어야 한다 (특정 실존 인물/특정 작품/특정 사물/특정 장소·사건의 고유한 이름). 가상·창작·지어낸 대상은 절대 금지.',
    '1-1. [★일반명사·장르·개념 금지★] 정답으로 장르·범주·개념·집합·현상을 가리키는 일반명사를 절대 고르지 마라 (예: "다이카이쥬(거대괴수)"·"좀비 영화"·"르네상스 미술"·"포유류"처럼 여러 대상을 아우르는 범주어 ✕). 반드시 그 범주 안의 "구체적 고유명사 하나"를 골라라 (예: 범주 "거대괴수"가 아니라 작품/캐릭터 "고지라" ○).',
    '1-2. [★유일 지목 검증★] 힌트들이 의도한 정답 "하나만" 유일하게 가리키는지 반드시 확인하라. 만약 작성한 힌트들이 정답보다 더 유명하거나 더 전형적인 다른 구체 대상에도 똑같이 들어맞는다면(예: 힌트가 사실상 "고지라"를 묘사하는데 정답을 "거대괴수"로 둠), 그 더 전형적인 대상을 정답으로 바꾸거나, 의도한 정답만 가리키도록 힌트를 다시 써라. 플레이어가 가장 자연스럽게 떠올릴 답이 오답 처리되는 문제를 절대 만들지 마라.',
