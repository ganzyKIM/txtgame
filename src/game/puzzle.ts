...
export function buildSetupPrompt(categoryLabel: string, theme: string, difficulty: Difficulty, recentAnswers: string[] = [], categoryDetail = ''): string {
  ...
  return [
    ...
    '2. [추측 가능성] 정답은 "해당 분야에 관심 있는 사람이라면 힌트를 보고 떠올릴 수 있을 만큼" 충분히 유명한 대상이어야 한다. 위키백과에 독립 항목으로 실릴 정도가 기준이다. 아무도 모르는 마이너 딥컷, 검증조차 어려운 무명 대상은 정답으로 절대 고르지 마라 — 풀 수 없는 문제는 실패한 문제다.',
