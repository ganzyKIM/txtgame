...
export default function App() {
  ...
  /** 닫기 = 창을 접고 바탕화면(게임 선택)으로. 예전엔 초텐쨩이 아예 못 나가게
   *  막았지만, 게임이 여러 종류가 된 지금은 "다른 게임 고르기"가 정상 동선이다.
   *
   *  주의: 창을 접으면 <Window> 아래 게임 컴포넌트가 통째로 언마운트되므로
   *  진행 중이던 판(오목 기보·홀덤 핸드와 산 칩)은 복구되지 않는다. 홀덤은
   *  다시 들어갈 때 크레딧을 또 써서 칩을 사야 하므로 확인을 꼭 받아야 한다. */
  async function handleClose() {
    push('> 아직 더 놀아야 해~! 정말 끝낼 거야…? ♡');
    mascot.current?.event('close');
    const ok = await showConfirm({
      title: '게임 종료',
      message: '정말 게임을 종료할까? 진행 중인 판은 사라져!',
      confirmLabel: '종료',
      cancelLabel: '더 놀기',
      danger: true,
    });
  ...
  // 오목 멀티도 동일 규칙: 진입 전 닉네임을 정하게 하고, 취소하면 들어가지 않는다.
  async function handleEnterGomokuMulti() {
    const name = await showPrompt({
      title: '오목 닉네임',
      message: '멀티에서 다른 사람에게 보일 닉네임을 정해줘! (최대 16자)',
      defaultValue: myNickname,
  ...
  const statusText =
    ...
    : mode === 'gomoku' ? '⚫ 오목'
    : mode === 'gomoku-multi' ? (gomokuRoom ? `⚫ ${gomokuRoom.hostNickname}의 대국` : '⚫ 오목 · 멀티 대기실')
    : mode === 'holdem' ? '🃏 텍사스 홀덤'
    ...
    <>
      {minimized ? (
          ...
          onHome={
            ...
            : mode === 'gomoku-multi' ? () => {
              void showConfirm('대국을 나가시겠어요?').then((ok) => {
                if (!ok) return;
                void gomokuRoomRef.current?.leaveRoom();
                setMode('gomoku'); setGomokuRoom(null);
              });
            }
            : mode === 'holdem-multi' ? () => {
        ...
        >
          ...
          /* 설득게임은 아직 미완이라 일부러 데스크탑 아이콘을 두지 않는다.
             (진입 경로가 없는 건 버그가 아님 — 완성되면 아이콘을 붙일 것) */
          ) : mode === 'persuade' ? (
            <PersuadeGame
              mascot={mascot}
              push={push}
            ...
            gomokuRoom ? (
              ...
              <GomokuLobby
                ...
                myNickname={myNickname}
                mascot={mascot}
                onJoin={(r) => setGomokuRoom(r)}
                onExit={() => setMode('gomoku')}
              />
            )
          ) : mode === 'holdem' ? (
            <HoldemGame
